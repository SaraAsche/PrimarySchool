...
def plotDegreeDegreeConnection(graph, weight):
    ...
    if weight:
        for i,j in graph.edges():
            xdata.append(graph.degree(i, weight='weight')); ydata.append(graph.degree(j,weight ='weight'))
            xdata.append(graph.degree(j, weight='weight')); ydata.append(graph.degree(i,weight='weight'))
    else:
        for i,j in withoutGrade_day1.edges():
            xdata.append(graph.degree(i)); ydata.append(graph.degree(j))
            xdata.append(graph.degree(j)); ydata.append(graph.degree(i))
    '''
    ...
    sns.jointplot(x=xdata, y=ydata, kind='hex')
    plt.show()
